	...
	{:else if node.type === 'file'}
		<div class="node-content file" onclick={() => onToggleSelect(node.uiPath, !isSelectedProp, node.metadata)} title="Toggle selection">
			...
			<div class="file-info">
				...
				{#if node.type === 'file' && node.metadata.source === 'local' && constitutionStore.submissionStatuses[node.metadata.localStorageKey]}
				  {@const status = constitutionStore.submissionStatuses[node.metadata.localStorageKey].status}
				  <span class="submission-status {status}" title="Submission status: {status}">
				    {#if status === 'pending'}
				      ⏳ 
